...
// Really annoying libshell file structure
...
double run_simulation(const LibShell::MeshConnectivity& mesh,
	...
	auto obj_func = [&](const Eigen::VectorXd& var, Eigen::VectorXd* grad, Eigen::SparseMatrix<double>* hessian,
		...
		double energy =
			LibShell::ElasticShell<SFF>::elasticEnergy(mesh, pos, edge_DOFs, *mat, rest_state, grad,
                    hessian ? &hessian_triplets : nullptr,
                    psd_proj ? proj_type : LibShell::HessianProjectType::kNone);
	...
	OptSolver::NewtonSolver(obj_func, find_max_step, x0, num_steps, grad_tol, x_tol, f_tol, proj_type != LibShell::HessianProjectType::kNone, true,
							is_swap);
...
int main(int argc, char* argv[]) {
	...
	polyscope::state::userCallback = [&]() {
		...
		if (ImGui::CollapsingHeader("Parameters", ImGuiTreeNodeFlags_DefaultOpen)) {
			...
			ImGui::Combo("Formulation", &sffid, "Tan\0Sin\0\0");
		}

		if (ImGui::CollapsingHeader("Optimization", ImGuiTreeNodeFlags_DefaultOpen)) {
            ImGui::Combo("Hessian Projection", (int*)(&proj_type), "No Projection\0Max Zero\0Abs\0\0");
