...
Usage:
    python half_cylinder_render.py -- -i /path/to/mesh -o /path/to/output
    python half_cylinder_render.py -- -i /path/to/mesh -o /path/to/output --flat-shading
"""
...
from create_boundary_wireframe import create_boundary_wire_object

def parse_half_cylinder_render_arguments():
...
def main():
    ...
    # Parse arguments
    # ========================================
    args = parse_half_cylinder_render_arguments()
    ...
    # ========================================
    # rgba
    obj_color = (0.226, 0.358, 0.67, 1.0)
    
    print(f"Object color: {obj_color}")
    ...
    # Material
    setMat_doubleColor_with_wireframe_modifier(mesh, meshColor_top, meshColor_bottom, AOStrength=ao_strength, edgeThickness=0)

    # Create boundary-only wire object
    wire_mat = bpy.data.materials.get("WireframeBlack") or bpy.data.materials.new("WireframeBlack")
    ...
    boundary_obj = create_boundary_wire_object(
        mesh_obj=mesh,
        radius=edge_thickness * 0.5,  # thickness -> radius
        wire_material=wire_mat,
    )

    # Optional: render boundary in front (helps avoid z-fighting)
    boundary_obj.show_in_front = True
    ...
    # Save blend for first mesh, name as input_mesh_name.blend
    blend_path = output_image.parent / f"{input_mesh.stem}.blend"
