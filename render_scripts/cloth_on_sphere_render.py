...
def main():
    ...
    # 2. The actual cloth mesh is the largest connected component, where the other connected component is the sphere mesh
    # 3. Save the cloth mesh and sphere as two new mesh files
    # 4. As sphere is always the same, we can just use the same sphere mesh for all frames
    # ========================================
    ...
    preprocessed_folder.mkdir(parents=True, exist_ok=True)  # create the folder to save the preprocessed meshes
    cloth_mesh_files = []
    sphere_mesh_file = None
    for i, mesh_file in enumerate(mesh_files):
        print(f"  [{i+1}/{len(mesh_files)}] Preprocessing: {mesh_file.name}")
        result = None   
        # using meshlab to split the mesh into connected components
        if i == 0:
            sphere_mesh_file = preprocessed_folder / f"{mesh_file.stem}_sphere.ply"
            result = save_largest_and_minz_of_smallest(str(mesh_file), str(preprocessed_folder / f"{mesh_file.stem}_cloth.ply"), str(sphere_mesh_file))
        else:
            result = save_largest_and_minz_of_smallest(str(mesh_file), str(preprocessed_folder / f"{mesh_file.stem}_cloth.ply"))
        print(f"    Result: {result}")
        cloth_mesh_files.append(preprocessed_folder / f"{mesh_file.stem}_cloth.ply")
    
    mesh_files = cloth_mesh_files
    ...
    for i, mesh_file in enumerate(mesh_files):
        ...
        if sphere_mesh_file is None:
            print(f"    ERROR: Failed to load sphere mesh file, skipping...")
            sys.exit(1)
        ...
        # Material
        setMat_doubleColor_with_wireframe_modifier(mesh, meshColor_top, meshColor_bottom, AOStrength=ao_strength)
