"""
Custom render script for crush cone simulation using BlenderToolbox.

Uses utility functions from render_meshes_bt.py

Usage:
    python crush_cone_render.py -- -i /path/to/meshes -o /path/to/output
    python crush_cone_render.py -- -i /path/to/meshes -o /path/to/output --flat-shading
    python crush_cone_render.py -- -i /path/to/meshes -o /path/to/output --video --fps 30
"""
...
def parse_crush_cone_arguments():
    ...
    parser = argparse.ArgumentParser(description='Render crush cone meshes using BlenderToolbox')
...
def main():
    """Custom main function for crush cone rendering."""
    ...
    for i, mesh_file in enumerate(mesh_files):
        ...
        result = save_largest_and_minz_of_smallest(str(mesh_file), str(preprocessed_folder / f"{mesh_file.stem}_cone.ply"), str(preprocessed_folder / f"{mesh_file.stem}_plane.ply"))
        print(f"    Result: {result}")
        processed_mesh_files.append(preprocessed_folder / f"{mesh_file.stem}_cone.ply")
    ...
    for i, mesh_file in enumerate(mesh_files):
        ...
        if args.render_plane:
            ...
            if plane_mesh is None:
                ...
                continue
            setMat_doubleColor_with_wireframe_modifier(plane_mesh, meshColor_top, bt.colorObj((0.3, 0.3, 0.3, 1.0), 0.5, 1.0, 1.0, 0.0, 0.0), AOStrength=1.0)
